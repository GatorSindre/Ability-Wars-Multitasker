...
addcmd('kidnapset',{'to'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
            ...
            local WPName = "kidnap"
            if getRoot(Players[v].Character) then
                notify('Modified Waypoints',"Created waypoint: "..getstring(1, args))
                local torso = getRoot(Players[v].Character)
                WayPoints[#WayPoints + 1] = {NAME = WPName, COORD = {math.floor(torso.Position.X), math.floor(torso.Position.Y), math.floor(torso.Position.Z)}, GAME = PlaceId}
                if AllWaypoints ~= nil then
                    AllWaypoints[#AllWaypoints + 1] = {NAME = WPName, COORD = {math.floor(torso.Position.X), math.floor(torso.Position.Y), math.floor(torso.Position.Z)}, GAME = PlaceId}
                end
...
addcmd('kidnap',{'ptp'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
			...
			getRoot(speaker.Character).CFrame = getRoot(Players[v].Character).CFrame + Vector3.new(0,-2,0)
			wait(0.1)
			getRoot(speaker.Character).CFrame = getRoot(Players[v].Character).CFrame + Vector3.new(0,-2,0)
...
addcmd('grounddeep',{'to'},function(args, speaker)
	local players = getPlayer(args[1], speaker)
	for i,v in pairs(players)do
		if Players[v].Character ~= nil then
			if speaker.Character:FindFirstChildOfClass('Humanoid') and speaker.Character:FindFirstChildOfClass('Humanoid').SeatPart then
            ...
            local targetPos = getRoot(Players[v].Character).Position
            local targetRot = getRoot(Players[v].Character).CFrame.Rotation

            getRoot(speaker.Character).CFrame = CFrame.new(targetPos.X, -3, targetPos.Z) * targetRot
		end
	end
	execCmd('breakvelocity')
end)
